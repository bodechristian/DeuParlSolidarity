...
OUTPUT_NAME = f"lr{LEARNING_RATE}-eps{EPSILON}-bs{BATCH_SIZE}-epochs{EPOCHS}-noDA"
...
def main(do_train=False):
    ...
    for i in range(3):
        ...
        train_inputs, test_inputs, train_labels, test_labels = train_test_split(input_ids, labels, random_state=2021+i,
                                                                                test_size=0.1)
        train_masks, test_masks, _, _ = train_test_split(attention_masks, input_ids, random_state=2021+i, test_size=0.1)

        # split train into train + val
        train_masks, val_masks, _, _ = train_test_split(train_masks, train_inputs, random_state=2021+i, test_size=0.1)
        train_inputs, val_inputs, train_labels, val_labels = train_test_split(train_inputs, train_labels,
                                                                              random_state=2021+i, test_size=0.1)
        ...
        test_dataloader = get_dataloader(test_inputs, test_labels, test_masks)

        print("Finished setting up data.")

        if do_train:
            train(train_dataloader, val_dataloader, i)
        test(test_dataloader, len(train_inputs), i)


def train(train_dataloader, val_dataloader, model_num):
    # model
    ...
    Path(join(getcwd(), "model", OUTPUT_NAME)).mkdir(parents=True, exist_ok=True)
    plt.savefig(join(getcwd(), "model", OUTPUT_NAME, f"{OUTPUT_NAME}_{model_num}"))
    torch.save(model.state_dict(), join(getcwd(), "model", OUTPUT_NAME, f"{OUTPUT_NAME}_{model_num}.bin"))

    #plt.show()


def test(test_dataloader, nb_train_data, model_num):
    # model
    model = BertForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=3)
    model.load_state_dict(torch.load(join(getcwd(), "model", OUTPUT_NAME, f"{OUTPUT_NAME}_{model_num}.bin")))
    model.cuda()
    model.eval()
    ...
    # Performance prints
    print(f"\nModel:\t{OUTPUT_NAME}")
    print(f"trainsize: {nb_train_data}")
    print(f"Accuracy: {accuracy_score(flat_labels, flat_preds)}")
